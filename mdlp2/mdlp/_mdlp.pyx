...
def MDLPDiscretize(np.ndarray[np.float64_t, ndim=1] col, np.ndarray[np.int64_t, ndim=1] y, int min_depth):
    ...
    cppsort(cut_loc.begin(), cut_loc.end())
    cut_points = np.array([get_cut(col, l) for l in cut_loc])
    return np.array(cut_loc), np.array(cut_points)

cdef unwrap(LEVEL level):
    return level[0], level[1], level[2]

@cython.boundscheck(False)
cdef double get_cut(np.ndarray[np.float64_t, ndim=1] col, int ind):
...
cdef bint reject_split(np.ndarray[np.int64_t, ndim=1] y, int start, int end, int k):
    ...
    cdef double N = <double> (end - start)
    ...
    cdef:
        ...
        double gain = whole_entropy - part1
        double entropy_diff = k0 * whole_entropy - k1 * entropy1 - k2 * entropy2
        double delta = log(pow(3, k0) - 2) - entropy_diff
    return gain <= 1 / N * (log(N - 1) + delta)

@cython.boundscheck(False)
def find_cut(np.ndarray[np.float64_t, ndim=1] x, np.ndarray[np.int64_t, ndim=1] y, int start, int end):
    ...
    cdef:
        ...
        double prev_entropy = np.inf  #INFINITY
        SIZE_t k = -1
        int ind
    ...
    for ind in range(start + 1, end):
        ...
        first_half = (<double> (ind - start)) / length \
                            * slice_entropy(y, start, ind)[0]
        second_half = (<double> (end - ind)) / length \
